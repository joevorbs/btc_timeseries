...
#Amount of time to pre-lag data in terms of days
lags = 730
...
col_list = new_df.columns.remove["Closing Price (USD)", "ds"]]
...
new_df['Closing Price (USD)'] = btc_interp['Closing Price (USD)'][0:len(new_df)]

#Isolate features & target
X = final_df.drop("Closing Price (USD)", axis = 1)
y = final_df['Closing Price (USD)']
...
lasso_cv = LassoCV(cv = 5, n_alphas = 100, max_iter = 1000) 

#Obtain coefficients 
coefs = pd.DataFrame(list(zip(lasso_cv.coef_, X_train.columns))).rename(columns = {0 : "Beta", 1 : "Feature"})
#Filter out any beta values shrunk to 0
coefs_final = coefs[coefs["Beta"] > 0]

##Build model with features chosen by regularization